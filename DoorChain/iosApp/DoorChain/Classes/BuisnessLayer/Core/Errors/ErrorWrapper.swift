 ...
 */
public enum ResultHandler<T, Error: Swift.Error>: CustomStringConvertible, CustomDebugStringConvertible {
    case success(T)
    case failure(Error)
    ...
    public init(_ value: T?, failWith: @autoclosure () -> Error) {
        self = value.map(ResultHandler.success) ?? .failure(failWith())
    }
